...
const TIERS = [
  ...
  { name: 'Silver', key: 'SILVER', min: 500, max: 999, color: 'from-gray-400 to-gray-600', textColor: 'text-gray-500', icon: Star, benefits: ['Priority support', 'Early booking access', '2% bonus points'] },
...
export const LoyaltyPage: React.FC = () => {
  ...
  const getTxIcon = (type: string) => {
    switch (type) {
      case 'EARNED': return <ArrowUp size={16} className="text-green-500" />;
      case 'REDEEMED': return <ArrowDown size={16} className="text-red-500" />;
      case 'BONUS': return <Gift size={16} className="text-purple-500" />;
      case 'REFERRAL': return <Users size={16} className="text-blue-500" />;
      default: return <Star size={16} className="text-yellow-500" />;
    }
  };
    ...
    <div className="flex flex-col min-h-screen bg-primary">
      ...
      <div className="flex-1 overflow-y-auto pb-20">
        ...
        <div className="px-4 pt-4 pb-2">
          <div className={`bg-gradient-to-br ${currentTier.color} rounded-2xl p-5 text-white shadow-lg`}>
        ...
        <div className="px-4 py-3 overflow-x-auto">
          <div className="flex bg-secondary rounded-xl p-1 min-w-max">
        ...
        <div className="px-4 space-y-3">
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <Card className="text-center py-4">
                  ...
                  <div className="text-lg font-bold text-primary">{status?.points || 0}</div>
                  <div className="text-xs text-secondary">Current Points</div>
                </Card>
                <Card className="text-center py-4">
                  <Award size={24} className={`${currentTier.textColor} mx-auto mb-1`} />
                  <div className="text-lg font-bold text-primary">{currentTier.name}</div>
                  <div className="text-xs text-secondary">Current Tier</div>
                </Card>
              </div>

              <Card>
                <h3 className="font-semibold text-primary mb-3">How to earn points</h3>
                <div className="space-y-2.5">
                  {[
                    { icon: <Star size={16} className="text-yellow-500" />, text: 'Complete a booking', pts: '+10 pts' },
                    { icon: <Users size={16} className="text-blue-500" />, text: 'Refer a friend', pts: '+50 pts' },
                    { icon: <TrendingUp size={16} className="text-green-500" />, text: 'Leave a review', pts: '+5 pts' },
                  ].map((item, i) => (
                    <div key={i} className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-secondary rounded-lg flex items-center justify-center">{item.icon}</div>
                      <span className="flex-1 text-sm text-primary">{item.text}</span>
                      <span className="text-xs font-semibold text-accent">{item.pts}</span>
                    </div>
                  ))}
                </div>
              </Card>
                    ...
                    <Card key={tx.id} className="mb-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-primary truncate">{tx.reason}</p>
                          <p className="text-xs text-secondary">{new Date(tx.createdAt).toLocaleDateString()}</p>
                        </div>
                        <span className={`text-sm font-semibold ${tx.type === 'REDEEMED' ? 'text-red-500' : 'text-green-500'}`}>
          ...
          {activeTab === 'history' && (
            history.length === 0 ? (
              <Card className="text-center py-12">
                ...
                <p className="text-secondary">No loyalty activity yet</p>
              </Card>
            ) : (
              history.map(tx => (
                <Card key={tx.id}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-primary truncate">{tx.reason}</p>
                      <p className="text-xs text-secondary">{new Date(tx.createdAt).toLocaleDateString()}</p>
                    </div>
                    <div className="text-right">
                      <span className={`text-sm font-bold ${tx.type === 'REDEEMED' ? 'text-red-500' : 'text-green-500'}`}>
          ...
          {activeTab === 'tiers' && (
            TIERS.map(tier => {
              ...
              return (
                <Card key={tier.key} className={isCurrent ? 'ring-2 ring-accent' : ''}>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {isCurrent && (
                          <span className="px-2 py-0.5 bg-accent/10 text-accent text-xs rounded-full font-medium">Current</span>
                        )}
                      </div>
          ...
          {activeTab === 'rewards' && (
            rewards.length === 0 ? (
              <Card className="text-center py-12">
                <Gift size={40} className="text-secondary mx-auto mb-3" />
                <p className="text-primary font-medium">No rewards available</p>
                <p className="text-secondary text-sm mt-1">Keep earning points to unlock rewards</p>
              </Card>
            ) : (
              rewards.map(reward => (
                <Card key={reward.id}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-semibold text-primary">{reward.title}</h3>
                      <p className="text-xs text-secondary truncate">{reward.description}</p>
                      <p className="text-xs font-medium text-accent mt-0.5">{reward.pointsCost} pts</p>
                    </div>
                    <Button
                      size="sm"
