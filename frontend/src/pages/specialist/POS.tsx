...
const POS: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 pb-28">
          ...
          {filtered.map((p) => {
            ...
            return (
              <button
                ...
                onClick={() => addToCart(p)}
                disabled={stock <= 0}
                className="text-left rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 hover:border-primary-400 hover:shadow-md transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {p.imageUrl ? (
                  <img src={p.imageUrl} alt="" className="w-full h-20 object-cover rounded-xl mb-2" />
                ) : (
                ...
                <div className="flex items-center justify-between mt-0.5">
                  <span className="text-sm font-semibold text-primary-600 dark:text-primary-400">{formatPrice(priceOf(p), p.currency as any)}</span>
                  <span className={`text-[11px] ${stock <= 0 ? 'text-red-500' : 'text-gray-400'}`}>{stock} {t('pos.inStock') || 'in stock'}</span>
                </div>
              </button>
            );
          })}
      ...
      {/* Sticky cart bar */}
      {itemCount > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-gray-900/95 backdrop-blur border-t border-gray-200 dark:border-gray-700 p-3 sm:p-4">
          <div className="max-w-3xl mx-auto flex items-center gap-3">
            <button onClick={() => setCart([])} className="text-sm text-gray-500 dark:text-gray-400 hover:text-red-500 px-2">{t('pos.clear') || 'Clear'}</button>
            <div className="flex-1 text-right">
              <span className="text-xs text-gray-500 dark:text-gray-400">{itemCount} {t('pos.items') || 'items'}</span>
              <div className="text-lg font-bold text-gray-900 dark:text-white">{formatPrice(total, currency as any)}</div>
            </div>
            <button onClick={() => { setCheckoutOpen(true); }} className="px-5 py-3 rounded-xl bg-primary-600 hover:bg-primary-700 text-white font-semibold">
      ...
      {checkoutOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="w-full sm:max-w-md bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('pos.reviewSale') || 'Review sale'}</h3>
              <button onClick={() => setCheckoutOpen(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-xl">×</button>
            </div>
              ...
              {cart.map((l) => (
                <div key={l.product.id} className="flex items-center gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">{formatPrice(priceOf(l.product), l.product.currency as any)}</div>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => setQty(l.product.id, l.qty - 1)} className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200">−</button>
                    <span className="w-8 text-center text-sm font-medium text-gray-900 dark:text-white">{l.qty}</span>
                    <button onClick={() => setQty(l.product.id, l.qty + 1)} className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200">+</button>
                  </div>
                  <div className="w-20 text-right text-sm font-semibold text-gray-900 dark:text-white">{formatPrice(priceOf(l.product) * l.qty, l.product.currency as any)}</div>
                </div>
              ))}
            </div>
            <div className="px-4 pb-2 flex items-center justify-between border-t border-gray-100 dark:border-gray-700 pt-3">
              ...
              <span className="text-xl font-bold text-gray-900 dark:text-white">{formatPrice(total, currency as any)}</span>
            </div>
            <div className="p-4">
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                {(['CASH', 'CARD', 'OTHER'] as PayMethod[]).map((m) => (
                  <button key={m} onClick={() => setPayMethod(m)}
                    className={`py-2.5 rounded-xl text-sm font-medium border transition-colors ${payMethod === m ? 'bg-primary-600 text-white border-primary-600' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600'}`}>
                    {t(`pos.pay.${m.toLowerCase()}`) || m}
                  </button>
                ))}
              </div>
              <button onClick={completeSale} disabled={submitting}
                className="w-full py-3 rounded-xl bg-green-600 hover:bg-green-700 text-white font-semibold disabled:opacity-60">
      ...
      {/* Receipt */}
      {receipt && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 print:bg-white">
          <div className="w-full max-w-xs bg-white dark:bg-gray-800 rounded-2xl p-5 print:shadow-none" id="pos-receipt">
            ...
            <div className="space-y-1 border-t border-dashed border-gray-300 dark:border-gray-600 pt-2">
              {(receipt.items || []).map((it: any) => (
                <div key={it.id} className="flex justify-between text-sm text-gray-800 dark:text-gray-200">
                  <span className="truncate">{it.name} × {num(it.quantity)}</span>
                  <span>{formatPrice(num(it.unitPrice) * num(it.quantity), receipt.currency)}</span>
                </div>
              ))}
            </div>
            <div className="flex justify-between font-bold text-gray-900 dark:text-white border-t border-dashed border-gray-300 dark:border-gray-600 mt-2 pt-2">
              ...
              <span>{formatPrice(num(receipt.total), receipt.currency)}</span>
            </div>
            <div className="flex gap-2 mt-4 print:hidden">
              <button onClick={() => window.print()} className="flex-1 py-2.5 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm font-medium">{t('pos.print') || 'Print'}</button>
              <button onClick={() => setReceipt(null)} className="flex-1 py-2.5 rounded-xl bg-primary-600 text-white text-sm font-medium">{t('pos.newSale') || 'New sale'}</button>
            </div>
          </div>
        </div>
      )}
