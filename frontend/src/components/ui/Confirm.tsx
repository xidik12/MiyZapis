    ...
    <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="confirm-title">
      <div ref={panelRef} className="modal-content" style={{ width: '100%', maxWidth: 420 }} tabIndex={-1}>
        <div className="modal-header">
          ...
          <button aria-label="Close dialog" className="btn btn-ghost" onClick={() => onResolve(false)}>
            <span aria-hidden>✕</span>
          </button>
        </div>
        <div className="modal-body text-secondary-content text-sm">
          {message}
          <div className="mt-3 text-xs text-gray-500 dark:text-gray-400">Press Esc to close</div>
        </div>
        <div className="modal-footer">
          <button className="btn btn-ghost" onClick={() => onResolve(false)}>{cancelText}</button>
          <button
            className={
              variant === 'destructive'
