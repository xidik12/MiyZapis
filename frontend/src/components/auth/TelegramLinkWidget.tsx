...
const TelegramLinkWidget: React.FC<TelegramLinkWidgetProps> = ({ onSuccess, onError }) => {
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      ...
      const msg = message || 'Failed to generate link code';
      ...
      <div className="w-full space-y-4">
        ...
        <div className="space-y-2">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 py-2.5 px-5 bg-[#54a9eb] hover:bg-[#4a96d2] text-white text-sm font-medium rounded-xl transition active:scale-[0.96]"
          >
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handleCopy}
              className="px-3 py-2.5 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 rounded-xl text-sm transition active:scale-[0.96]"
            >
        ...
        <div className="flex items-center gap-4 pt-1">
          <button
            ...
            className="text-sm text-primary-600 dark:text-primary-400 hover:underline font-medium transition active:scale-[0.96]"
          >
            {t('telegram.link.connected')}
          </button>
          <button
            onClick={handleGenerateLink}
            className="text-sm text-gray-500 dark:text-gray-400 hover:underline transition active:scale-[0.96]"
          >
  ...
  return (
    <div className="w-full">
      ...
      <button
        onClick={handleGenerateLink}
        disabled={isLoading}
        className="inline-flex items-center gap-2 py-2 px-4 bg-[#54a9eb] hover:bg-[#4a96d2] disabled:opacity-50 text-white text-sm font-medium rounded-xl transition active:scale-[0.96] disabled:active:scale-100"
      >
        {isLoading ? (
          <>
